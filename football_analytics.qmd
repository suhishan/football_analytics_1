...
It is clear from the joint distribution of attack and defense parameters that in both the complete-pooling and the hierarchical model that these features are covary i.e. the plot above shows teams with better attack also have better defences. This leads us to the consideration of another model, where we allow try and estimate the madgnitude of such covariance and whether this added information aids us in better predictions.
...
```
Estimating the correlation paramter Rho:
...
## Comparison of features: Hierarchical model and Correlated Features Model.
...
tibble(
  att = c(d_corr$att, d_m2$att2),
  def = c(d_corr$def, d_m2$def2),
  Model = factor(rep(c("Covarying","Multilevel"), each = 20)),
  team = factor(rep(seq(1, 20), 2))
) %>% ggplot(aes(x = att, y = def))+
  geom_point(aes(color = Model), size = 3)+
  scale_color_manual(
  ...
  #annotate("text", -0.5, -0.05, label = expression(mu[def]))+
  #annotate("text", 0.15, 0.75, label = expression(mu[att]))+
  labs(title = "Comparison between multilevel model and correlated features model")+
